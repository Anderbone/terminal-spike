...
The earlier fold-unavailable blocker was resolved by the completed deployment below.
The user's visual acceptance remains pending; the living smooth-scroll record is not marked solved.

## Completed fold deployment

2026-09-07T22:11:34+00:00: final fold deployment completed. Paired using the user-provided one-time code (not retained in this record), resolved exact connected wireless serial `192.168.0.33:36441` from `adb devices -l`, and verified `SM-F976B` before install. Latest source `test lint assembleDebug --max-workers=2 --no-configuration-cache` passed (`build/tmux-status-fold-install-gates.log`, 166 tasks). Debug APK SHA-256 `4f7b6a651ddcb8f54c5a21eaf06ff4c8ed726d6c5bd6184e12312b59ee7f9f6e` installed successfully with `adb install -r`. `am start -W` returned Status ok; MainActivity was top-resumed at launch and on a later independent check. Deployment evidence: `build/tmux-status-fold-deployment.json`. **No tests ran on the fold.** Coding, automated verification and requested deployment are complete; the user's visual confirmation of the real scrolling and task badges is still pending. Do not mark the living record solved until that confirmation.
