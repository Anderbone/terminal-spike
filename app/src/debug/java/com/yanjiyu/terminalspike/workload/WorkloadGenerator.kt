...
class WorkloadGenerator(
        ...
        return when ((sequence % SAMPLE_VARIANTS).toInt()) {
            ...
            5 -> TerminalLine.styled(
                listOf(
                    TerminalRun(
                        "PASS ",
                        TerminalStyle(foreground = TerminalColour.Rgb(TerminalPalette.GREEN), bold = true),
                    ),
            ...
            9 -> TerminalLine.styled(
                listOf(
                    TerminalRun(
                        "error ",
                        TerminalStyle(foreground = TerminalColour.Rgb(TerminalPalette.RED), bold = true),
                    ),
                    TerminalRun(
                        "symbol not found",
                        TerminalStyle(foreground = TerminalColour.Rgb(TerminalPalette.RED), underline = true),
                    ),
            ...
            13 -> TerminalLine.styled(
                listOf(
                    TerminalRun(
                        "tmux ",
                        TerminalStyle(
                            background = TerminalColour.Rgb(TerminalPalette.BLUE),
                            foreground = TerminalColour.Rgb(TerminalPalette.WHITE),
                            bold = true,
                        ),
                    ),
    ...
    fun fullScreen(update: Long, rows: Int = DEFAULT_SCREEN_ROWS): GeneratedScreen {
        ...
        repeat(clampedRows - 6) { row ->
            ...
            val style = if (active) {
                TerminalStyle(
                    background = TerminalColour.Rgb(TerminalPalette.SELECTION_MUTED),
                    foreground = TerminalColour.Rgb(TerminalPalette.WHITE),
                    bold = true,
                )
            } else {
                TerminalStyle(
                    foreground = if (row % 3 == 0) {
                        TerminalColour.Rgb(TerminalPalette.CYAN)
                    } else {
        ...
        result += TerminalLine.styled(
            listOf(
                ...
                TerminalRun("  q Quit  "),
                TerminalRun(
                    "updates=$update  cursor moves without a parser",
                    TerminalStyle(foreground = TerminalColour.Rgb(TerminalPalette.GREEN)),
                ),
            ),
        )
    ...
    private fun styledPrefix(prefix: String, body: String, colour: Int): TerminalLine =
        TerminalLine.styled(
            listOf(
                TerminalRun(
                    prefix,
                    TerminalStyle(foreground = TerminalColour.Rgb(colour), bold = true),
                ),
                TerminalRun(body),
            ),
        )
