...
internal fun parseTmuxHistoryCapture(capture: TmuxPaneCapture): TmuxLocalHistorySnapshot? {
    ...
    if (capture.historyRows == 0 || !capture.historyIncluded) {
        return TmuxLocalHistorySnapshot(
            ...
            lines = emptyList(),
            remoteHistoryRows = capture.remoteHistoryRows,
            capturedStartRow = capture.capturedStartRow,
            oldestAvailableRow = capture.oldestAvailableRow,
            olderPage = capture.olderPage,
