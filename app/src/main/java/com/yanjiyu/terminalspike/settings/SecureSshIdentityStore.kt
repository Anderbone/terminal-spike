...
class SecureSshIdentityStore(
    ...
    fun load(identityId: Long): ByteArray {
        ...
        val encrypted = file.openRead().use { input ->
            require(input.channel.size() in 1..MAX_ENCRYPTED_BYTES.toLong()) {
                "Invalid SSH identity file size."
            }
    ...
    private fun getExistingKey(): SecretKey? {
        ...
        return keyStore.getKey(keyAlias, null) as? SecretKey
            ?: throw StoredCredentialKeyUnavailableException(StoredCredentialKind.SSH_PRIVATE_KEY)
    }
